...
uint64_t part_a_opencl(const CONFIG *config)
{
    ...
    cl_kernel        ko_part_a;       // compute kernel
    ...
    {
        int error = clGetPlatformIDs(numPlatforms, Platform, NULL);
        if (error != 0)
        {
    ...
    {
        int error = 0;
        ko_part_a = clCreateKernel(program, "kernel_part_a", &error);
        if (error)
        {
    ...
    {
        int error = 0;
        error  = clSetKernelArg(ko_part_a, 0, sizeof(cl_mem), &device_seeds);
        error |= clSetKernelArg(ko_part_a, 1, sizeof(cl_ulong), &num_seeds);
        error |= clSetKernelArg(ko_part_a, 2, sizeof(cl_mem), &device_seed_map_layers_sizes);
        error |= clSetKernelArg(ko_part_a, 3, sizeof(cl_mem), &device_flat_seed_map_layers);
        error |= clSetKernelArg(ko_part_a, 4, sizeof(cl_ulong), &num_seed_map_layers);
        error |= clSetKernelArg(ko_part_a, 5, sizeof(cl_ulong), &total_size);
        error |= clSetKernelArg(ko_part_a, 6, sizeof(cl_mem), &device_results);
    ...
    // letting the OpenCL runtime choose the work-group size
    {
        ...
        error = clEnqueueNDRangeKernel(commands, ko_part_a, 1, NULL, &global, NULL, 0, NULL, NULL);
        if (error)
        {
