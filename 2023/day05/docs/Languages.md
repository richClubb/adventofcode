...
* C OpenMP - Shockingly simple to figure out, very cool.
* C OpenCL - Similar to CUDA but harder to understand the docs and public information, probably why people like CUDA so much.
...
I tried to set up devcontainers for the project so that it's easier to manage dependencies. I would have prefered to have a pure Dockerfile definition but vscode's use of the devcontainer.json and the features is nice.

### Project organisation

I've tried to develop these examples with a heavy focus on TDD  with high levels of unit test coverage, each 'module' is it's own folder in the `src` tree and this contains all the associated source and includes (where appropriate) as well as the test code for each module. I wasn't very strict on this, especially when I got further on as I wanted to make progress, but it was good to be able to test things when they were frustrating, and having a good structure meant this was very easy.
...
There are also constructs for; Seeds, Seed Ranges, Seed Maps and Seed Map Layers to make dealing with the arrays easier. This is more heavily used in some languages than others.
...
I have tried to set up some debug targets in the `.vscode` folder for each project so that they can be reused inside the devcontainers, but this fell apart a bit in places as it's a lot of boilerplate.
...
I used this as an excuse to heavily use memory management as it's not something I'm very comfortable using, and I made sure to use 'valgrind' heavily during unit and system testing as well as the `-fsanitize=address` to make sure I validated the different pieces of code cleaned up their memory correctly.

I want to try and find a better method of unit testing the "memory safety", but doing this kind of work does make you keep track of the entities you create and get a good mental picture of what you're trying to do. I tried to declare parameters as `const` where possible to minimise the mutability of the system.
...
I'm having to do more C++ for my job at the moment, and I'm finding it pretty pleasant as there are some quality of life improvements over C, especially in `vectors` and other return types like `optional`
...
My solution 'works' but isn't very good and could definitely use some more work to make it good. I'd expect the speed difference to be staggering as I am processing the data in 3072 cores rather than just 28, even if they are slower.
