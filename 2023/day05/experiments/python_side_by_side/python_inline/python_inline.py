...
def process_seed(seed: int, layers: list):
    ...
    for layer in layers:
        for map in layer:
            ...
            if val >= source_start and val < source_end:
...
def process_seed_range(seed_range: tuple, layers: list):
    start = seed_range[0]
...
def part_a(input_file_path: str):
    ...
    # small number of seeds so this is a nice abstraction
    for seed in seeds:
...
def part_b(input_file_path: str):
    ...
    # Small number of seed ranges so this is a nice abstraction
    for seed_range in seed_ranges:
