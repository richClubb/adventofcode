...
pub fn build(b: *std.Build) void {
    ...
    const seed_range = b.addModule("seed_range", .{
        ...
        // of this module, you will have to make sure to re-export them from
        // the root file.
        .root_source_file = b.path("src/seed_range/seed_range.zig"),
        // Later on we'll use this module as the root module of a test executable
        // which requires us to specify a target.
        .target = target,
    });

    const seed_map = b.addModule("seed_map", .{
        // The root source file is the "entry point" of this module. Users of
        // this module will only be able to access public declarations contained
        // in this file, which means that if you have declarations that you
        // intend to expose to consumers that were defined in other files part
        // of this module, you will have to make sure to re-export them from
        // the root file.
        .root_source_file = b.path("src/seed_map/seed_map.zig"),
        // Later on we'll use this module as the root module of a test executable
        // which requires us to specify a target.
        .target = target,
    });

    const seed_map_layer = b.addModule("seed_map_layer", .{
        // The root source file is the "entry point" of this module. Users of
        // this module will only be able to access public declarations contained
        // in this file, which means that if you have declarations that you
        // intend to expose to consumers that were defined in other files part
        // of this module, you will have to make sure to re-export them from
        // the root file.
        .root_source_file = b.path("src/seed_map_layer/seed_map_layer.zig"),
        // Later on we'll use this module as the root module of a test executable
        // which requires us to specify a target.
        .target = target,
        .imports = &.{
    ...
    const part_b = b.addModule("part_b", .{
        // The root source file is the "entry point" of this module. Users of
        // this module will only be able to access public declarations contained
        // in this file, which means that if you have declarations that you
        // intend to expose to consumers that were defined in other files part
        // of this module, you will have to make sure to re-export them from
        // the root file.
        .root_source_file = b.path("src/part_b/part_b.zig"),
        // Later on we'll use this module as the root module of a test executable
        // which requires us to specify a target.
        .target = target,
        .imports = &.{
    ...
    const part_a_tests = b.addTest(.{
        .root_module = part_a,
    });

    const part_b_tests = b.addTest(.{
        .root_module = part_b,
    });

    const seed_map_tests = b.addTest(.{
        .root_module = seed_map,
    });

    const seed_map_layer_tests = b.addTest(.{
    ...
    const run_part_a_tests = b.addRunArtifact(part_a_tests);
    ...
    test_step.dependOn(&run_part_b_tests.step);
    test_step.dependOn(&run_part_a_tests.step);
