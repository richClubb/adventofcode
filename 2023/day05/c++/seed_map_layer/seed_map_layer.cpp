...
uint64_t SeedMapLayer::map_seed_opt(uint64_t input)
{
    for(SeedMap &seed_map : this->seed_maps)
    {
        if (std::optional<uint64_t> result; (result = seed_map.map_seed_opt(input)) != std::nullopt)
        {
            return result.value();
        }
    }
...
void SeedMapLayer::sort_seed_maps()
{
    ...
    for (uint64_t index_i = 0; index_i < seed_maps_size - 1; index_i++) {
        swapped = false;
        for (uint64_t index_j = 0; index_j < seed_maps_size - index_i - 1; index_j++) {
