...
// impl Equipment {
//     pub fn new(equip_type_string: String, input: String) -> Self {
//         let equip_re = Regex::new(r"(?<name>[A-Za-z]{1,})\s{1,}(?<cost>[0-9]{1,})\s{1,}(?<attack>[0-9]{1,})\s{1,}(?<def>[0-9]{1,})").unwrap();

//         let (name, cost, attack, def) = equip_re.captures(&input).map(|caps| {
//             let name = caps.name("name").unwrap().as_str().to_string();
//             let cost = caps.name("cost").unwrap().as_str().parse::<usize>().unwrap();
//             let attack = caps.name("attack").unwrap().as_str().parse::<usize>().unwrap();
//             let def = caps.name("def").unwrap().as_str().parse::<usize>().unwrap();
 
//             (name, cost, attack, def)
//            }
//         ).unwrap(); 

//         let equip_type = match equip_type_string.as_str() {
//             "Weapons" => EquipmentType::Weapon,
//             "Armor" => EquipmentType::Armour,
