...
pub fn part_b(sue_list_path: &String, sue_info_path: &String)
{
    println!("Part B");
    ...
    let total_filter = sue_list.iter().filter(|item| {
            ((item.1.children == ref_sue_info.children) || item.1.children.is_none()) &&
            ((item.1.cats > ref_sue_info.cats) || item.1.cats.is_none() ) &&
            ((item.1.samoyeds == ref_sue_info.samoyeds) || item.1.samoyeds.is_none()) &&
            ((item.1.pomeranians < ref_sue_info.pomeranians) || item.1.pomeranians.is_none()) &&
            ((item.1.akitas == ref_sue_info.akitas) || item.1.akitas.is_none()) &&
            ((item.1.vizslas == ref_sue_info.vizslas) || item.1.vizslas.is_none()) &&
            ((item.1.goldfish < ref_sue_info.goldfish) || item.1.goldfish.is_none() ) &&
            ((item.1.trees > ref_sue_info.trees) || item.1.trees.is_none() ) && 
            ((item.1.cars == ref_sue_info.cars) || item.1.cars.is_none() ) && 
            ((item.1.perfumes == ref_sue_info.perfumes) || item.1.perfumes.is_none()) 
        }
    );
